...
class GetDetails:
    ...
    async def get_basic_info(self, session, webtoon_url):
        ...
        if self.storage == 'Local':
            self.download_data_locally(self, dict_of_webtoon_info, webtoon_url)
        elif self.storage == 'RDS':
            self.upload_data_to_RDS(self, dict_of_webtoon_info)
        else:
            self.download_data_locally(self, dict_of_webtoon_info, webtoon_url)
            self.upload_data_to_RDS(self, dict_of_webtoon_info)

    def download_data_locally(self, dict_of_webtoon_info, webtoon_url):
        if os.path.isfile(const.ALL_WEBTOONS_DIR_PATH + f'/{webtoon_url.split("/")[5]}/basic_info.json'):
            pass
        else:
    ...
    def upload_data_to_RDS(self, dict_of_webtoon_info):
        conn = None
